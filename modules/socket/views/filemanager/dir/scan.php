...
<div id="information">
    ...
    if($pathA == '.')
        ...
        $nav .= '/'.$pathA;
    ?>
    <a style="float:left;" href="/<?=Registry::i()->class_link?>/directory<?=Url::query(array('dir'=>Request::param($nav,TRUE)),'auto')?>"><?=($pathA = utf8_decode($pathA) AND(Str::charset($pathA) === 'ASCII'))? iconv('cp1251','UTF-8',$pathA) : $pathA ?>/</a>
    <?php
    endforeach;
    ?>
    ...
    if($back !== NULL):
    ?>
    <p class="back <?=(!isset($_GET['dir']) OR $_GET['dir'] == '.')?'hidden':''?>"><a href="/<?=Registry::i()->class_link?>/directory<?=Url::query(array('dir'=>Request::param($back,TRUE)),'auto')?>">Назад</a></p>
    <?php
    endif;
    ?>
...
<form method="post" id="main_form">
    <div id="directory" class="table">
        ...
        ?>
        <p class="table-row">
            <a class="table-cell name" href="/<?=Registry::i()->class_link?>/directory<?=Url::query(array('dir'=>Request::param($path.'/'.$directory['dir'],TRUE),'mode'=>NULL),'auto')?>"><?=$directory['name']?></a>
    ...
    <div id="file" class="table">
        ...
        ?>
        <p class="table-row">
            <a class="table-cell name" href="/<?=Registry::i()->class_link?>/file<?=Url::query(array('dir'=>Request::param($path),'file'=>Request::param($file['file']),'mode'=>NULL),'auto')?>"><?=$file['name']?></a>
            ...
            ?>
            <a href="<?=Core::$root_url.ltrim($path_utf.'/'.$file['name'],'.')?>" class="table-cell save_link">Скачать как ссылку</a>
            <?php
            endif;
            ?>
        </p>
