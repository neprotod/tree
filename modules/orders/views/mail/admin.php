...
<div class="main" style="width:760px">
    ...
    <p>
        <b>Электронная почта: </b><?=(!empty($order['email']))? $order['email'] : 'не предоставил' ?>
    </p>
    <p>
        <b>Город: </b><?=(!empty($order['address']))? $order['address'] : 'не предоставил' ?>
    </p>
    <p>
        <b>IP - адрес покупателя: </b><?=$order['ip']?>
    </p>
    <?php
    if(!empty($order['comment'])):
    ?>
    <p>
        <b>Коментарий: </b><?=$order['comment']?>
    </p>
    <?php
    endif;
    ?>
    <table id="purchases" style="border: 1px solid #CDCDCD; border-radius: 10px; padding: 10px; margin-bottom:50px; width: 650px;">
        <tbody>
            <tr>
                <td colspan="4">
