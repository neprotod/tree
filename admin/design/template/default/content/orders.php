...
<?php
$design->tabs('start');
echo Admin_Template::factory(Registry::i()->settings['admin_theme'],'content_tabs_orders',array('active'=>Request::param(Request::get('status'),NULL,0)));
$design->tabs('end');
?>
<!-- Вкладки END -->
            ...
            ?>
                <div class="<?=isset($order['paid'])? 'green' : '' ?> row">        
                    ...
                    <div class="order_name cell">
                        <a href="<?=Url::query_root(array('module'=>'order','id'=>$order['id'], 'return'=>urlencode(Url::root(NULL))),TRUE,'auto')?>">Заказ №<?=$order['id']?></a>
...
<div id="right_menu">
    ...
    <ul id="labels">
        <li <?=(!$label)? 'class="selected"' : ''?>>
            ...
            <a href="<?=URL::query_root(array('label'=>NULL),TRUE,'auto')?>">Все заказы</a>
        </li>
        <?php
        foreach($labels as $l):
        ?>
        <li data-label-id="<?=$l['id']?>" <?=($label['id'] == $l['id'])? 'class="selected"' : ''?>>
            <span style="background-color:#<?=$l['color']?>;" class="order_label"></span>
            <a href="<?=URL::query_root(array('label'=>$l['id']),TRUE,'auto')?>"><?=$l['name']?></a>
        </li>
        <?php
        endforeach;
        ?>
