...
class Admin_View {
     ...
     * вам необходимо вызвать [Admin_View::set_filename].
     *
     *     $view = Admin_View::factory($file);
     *
    ...
    public static function factory($file = NULL, $module = NULL, array $data = NULL){
        $view = new Admin_View($file, $module, $data);
     ...
     * области.
     *
     *     $output = Admin_View::capture($file, $data);
     *
    ...
    protected static function capture($core_view_filename, array $core_view_data)
    {
        ...
        if (Admin_View::$_global_data){
            // Импортировать глобальные переменные в локальное пространство имен
            extract(Admin_View::$_global_data, EXTR_SKIP);
        }
    ...
    /**
     * Устанавливает глобальную переменную, похож на [Admin_View::set], 
     * за исключением того, что переменная будет доступна для всех 
     * представлений.
     * 
     *
     *     Admin_View::set_global($name, $value);
     *
    ...
    public static function set_global($key, $value = NULL){
        if (is_array($key)){
            foreach ($key as $key2 => $value){
                Admin_View::$_global_data[$key2] = $value;
            }
        }else{
            Admin_View::$_global_data[$key] = $value;
        }
    }

    /**
     * Присваивает глобальную переменную по ссылке, похож на [Admin_View::bind], 
     * за исключением что переменная будет доступна для всех представлений.
     *
     *     Admin_View::bind_global($key, $value);
     *
     ...
     * Файл почти всегда должен загружатся с помощью [Admin_View::factory].
     *
    ...
    public function & __get($key){
        ...
        elseif (array_key_exists($key, Admin_View::$_global_data)){
            return Admin_View::$_global_data[$key];
        }
        else{
    ...
    /**
     * Магический метод, вызов [Admin_View::set] для загрузки переменных.
     *
     *     $view->foo = 'something';
     *
     * @param   string  variable name
    ...
    public function __isset($key){
        return (isset($this->_data[$key]) OR isset(Admin_View::$_global_data[$key]));
    }
    ...
    public function __unset($key){
        unset($this->_data[$key], Admin_View::$_global_data[$key]);
    }

    /**
     * Волшебный метод, возвращает выходные данные [Admin_View::render].
     *
     * @return  string
     * @uses    Admin_View::render
     */
    ...
    function set_filename($file, $module){
        ...
        if(isset($module))
            $path = Admin_Module::mod_path($module).$file.EXT;
        else 
            $path = $file.EXT;
    ...
    public function render($file = NULL){
        ...
        return Admin_View::capture($this->_file, $this->_data);
    }
